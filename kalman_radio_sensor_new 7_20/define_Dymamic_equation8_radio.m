...
%% format = 1 straight line ,format = 2 square
if (format_1 == 1)
...
phi_z = I+(F_z*dt);
%phi_z = I + F_z*dt; 
Q_z(2,2) = ((cos(psi_h(k-1))).^2)*sig_bx^2 + ((sin(psi_h(k-1))).^2)*sig_by^2 + yvm_Nh(k-1)*yvm_Nh(k-1)*sig_arw_0^2;
Q_z(3,3) = sig_xr^2;
Q_z(5,5) = ((sin(psi_h(k-1))).^2)*sig_bx^2 + ((cos(psi_h(k-1))).^2)*sig_by^2 + xvm_Nh(k-1)*xvm_Nh(k-1)*sig_arw_0^2;
Q_z(6,6) = sig_yr^2;
Q_z(7,7) = 1*sig_arw_0^2;
Q_z(8,8) = 1*sig_rrw_0^2;
