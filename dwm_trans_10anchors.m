...
% clear all;
% clc;
load('aj.mat');%linvelgyro125.mat  linveltest (7)  data (6)aj_1aj_1data (4)
% load('multisapce_data_1070305_v2.mat');
%aj_7.mat  (y_anchor_2 = 9.37; x_anchor_2 = 0.55;z_anchor_2 = 1.83;)
[row,count] = size(data);
if row == 44
    dis_len = 23;%11-4anchros  23-10anchros
    imu_len = 21;
    pos = zeros(10,count);
    pos_start_byte = 17;
else if row == 32
    dis_len = 11;%11-4anchros  23-10anchros
    imu_len = 21;  
    pos = zeros(4,count);  
    pos_start_byte = 17;
    end
...
for j_1=1:count_for
    for i_1=1:row 
        %% distance data
        if data(i_1,j_1) == pos_start_byte
            if i_1+dis_len-1 <= row && data(i_1+dis_len-1,j_1) == 128
                if rem(sum( data(i_1:(i_1+dis_len-3),j_1) ),256) == data(i_1+dis_len-2,j_1)
                    data_new(1:dis_len,distance_cnt) = data(i_1:(i_1+dis_len-1),j_1);
                    distance_cnt = distance_cnt+1;
                end
            end
            if i_1+dis_len-1 < row+2 && i_1+dis_len-1 > row && data(i_1-imu_len-1,j_1+1) == 128
                if  rem(sum( data(i_1:i_1+dis_len-3,j_1) ) ,256) == data(i_1+dis_len-2,j_1)                    
                    data_new(1:(row-i_1+1),distance_cnt) = data(i_1:row,j_1);
                    data_new((row-i_1+2):dis_len,distance_cnt) = data(1:(i_1-imu_len-1),j_1+1);
                    distance_cnt = distance_cnt+1;
                end
            end
            if i_1+dis_len-1 <= row+2 && i_1+dis_len-1 > row+1 && data(i_1-imu_len-1,j_1+1) == 128
                if  rem(sum( data(i_1:i_1+dis_len-3,j_1) ) ,256) == data(i_1-imu_len-2,j_1+1)                    
                    data_new(1:(row-i_1+1),distance_cnt) = data(i_1:row,j_1);
                    data_new((row-i_1+2):dis_len,distance_cnt) = data(1:(i_1-imu_len-1),j_1+1);
                    distance_cnt = distance_cnt+1;
                end
            ...
            if i_1+dis_len-1 > row+2 && data(i_1-imu_len-1,j_1+1) == 128
                if  rem(sum( data(i_1:row,j_1) ) + sum( data(1:(i_1-imu_len-3),j_1+1) ),256) == data((i_1-imu_len-2),j_1+1)
                    data_new(1:(row-i_1+1),distance_cnt) = data(i_1:row,j_1);
                    data_new((row-i_1+2):dis_len,distance_cnt) = data(1:(i_1-imu_len-1),j_1+1);
                    distance_cnt = distance_cnt+1;
                end                
        ...
        if data(i_1,j_1) == 8
            if i_1+imu_len-1 <= row && data(i_1+imu_len-1,j_1) == 128
                if  rem(sum( data(i_1:(i_1+imu_len-3),j_1) ),256) == data(i_1+imu_len-2,j_1);
                    data_new(dis_len+1:row,inertial_cnt) = data(i_1:(i_1+imu_len-1),j_1);
                    inertial_cnt = inertial_cnt+1;
                end
            end
            if i_1+imu_len-1 <= row+1 && i_1+imu_len-1 > row && data(i_1-dis_len-1,j_1+1) == 128
                if  rem(sum( data(i_1:(i_1+imu_len-3),j_1) ),256) == data(i_1+imu_len-2,j_1);
                    data_new(dis_len+1:(row+dis_len-i_1+1),inertial_cnt) = data(i_1:row,j_1);
                    data_new((row+dis_len-i_1+2):row,inertial_cnt) = data(1:(i_1-dis_len-1),j_1+1);
                    inertial_cnt = inertial_cnt+1;
                end                
            end       
            
            if i_1+imu_len-1 <= row+2 && i_1+imu_len-1 > row+1 && data(i_1-dis_len-1,j_1+1) == 128
                if  rem(sum( data(i_1:(i_1+imu_len-3),j_1) ),256) == data(i_1-dis_len-2,j_1+1);
                    data_new(dis_len+1:(row+dis_len-i_1+1),inertial_cnt) = data(i_1:row,j_1);
                    data_new((row+dis_len-i_1+2):row,inertial_cnt) = data(1:(i_1-dis_len-1),j_1+1);
                    inertial_cnt = inertial_cnt+1;
                end                
            end       
            if i_1+imu_len-1 > row+2 && data(i_1-dis_len-1,j_1+1) == 128
                if rem((sum( data(i_1:row,j_1) ) + sum( data(1:(i_1-dis_len-3),j_1+1))),256)== data((i_1-dis_len-2),j_1+1)
                    data_new(dis_len+1:(row+dis_len-i_1+1),inertial_cnt) = data(i_1:row,j_1);
                    data_new((row+dis_len-i_1+2):row,inertial_cnt) = data(1:(i_1-dis_len-1),j_1+1);
                    inertial_cnt = inertial_cnt+1;
                end                
...
for j_1=1:count_for
%     for i_1=1:30 
        gro(1,j_1) = (bitshift(data_new(dis_len+2,j_1),8)+data_new(dis_len+3,j_1));% gro x
        gro(2,j_1) = (bitshift(data_new(dis_len+4,j_1),8)+data_new(dis_len+5,j_1));% gro y
        gro(3,j_1) = (bitshift(data_new(dis_len+6,j_1),8)+data_new(dis_len+7,j_1));% gro z
        acc(1,j_1) = (bitshift(data_new(dis_len+8,j_1),8)+data_new(dis_len+9,j_1));% acc x
        acc(2,j_1) = (bitshift(data_new(dis_len+10,j_1),8)+data_new(dis_len+11,j_1));% acc y
        acc(3,j_1) = (bitshift(data_new(dis_len+12,j_1),8)+data_new(dis_len+13,j_1));% acc z  
        ...
        if row == 44
            pos(5,j_1) = (bitshift(data_new(10,j_1),8)+data_new(11,j_1))/1024;
            pos(6,j_1) = (bitshift(data_new(12,j_1),8)+data_new(13,j_1))/1024;
            pos(7,j_1) = (bitshift(data_new(14,j_1),8)+data_new(15,j_1))/1024;
            pos(8,j_1) = (bitshift(data_new(16,j_1),8)+data_new(17,j_1))/1024;
            pos(9,j_1) = (bitshift(data_new(18,j_1),8)+data_new(19,j_1))/1024;
            pos(10,j_1) = (bitshift(data_new(20,j_1),8)+data_new(21,j_1))/1024;
            pos(1,j_1) = pos(7,j_1);
            pos(2,j_1) = pos(8,j_1);
            pos(3,j_1) = pos(9,j_1);
            pos(4,j_1) = pos(10,j_1);            
        end
end
...
for j_1= 2:1:count_for
    ...
    if row == 44
        if(pos(5,j_1)>15)
            pos(5,j_1) = pos(5,j_1-1);
        end

        if(pos(6,j_1)>15)
            pos(6,j_1) = pos(6,j_1-1);
        end

        if(pos(7,j_1)>15)
            pos(7,j_1) = pos(7,j_1-1);
        end

        if(pos(8,j_1)>15)
            pos(8,j_1) = pos(8,j_1-1);
        end    

        if(pos(9,j_1)>15)
            pos(9,j_1) = pos(9,j_1-1);
        end

        if(pos(10,j_1)>15)
            pos(10,j_1) = pos(10,j_1-1);
        end 
    end
end
...
y_anchor_3 = 11.7;
x_anchor_3 = 7.78;%7.73 7.78     7.85
z_anchor_3 = 2.5;
...
x_tag = 3.06; y_tag = 6.75; z_tag = 1.32; %x_tag = 4.64
