function [Rm_data,Rm_Index,nvx_r,nvy_r] = radio_sensor_m_mult(coordinate,x_p_N,y_p_N,sig_x_r,sig_y_r,n,m)
    ...
    for i=1:n
        ...
        noise = [nvx_r(i) nvx_r(i) nvx_r(i) nvx_r(i) nvx_r(i) nverr(i)]';
        Rm(i,1:col) = sqrt(coor_diff(:,1).*coor_diff(:,1) + coor_diff(:,2).*coor_diff(:,2) + coor_diff(:,3)) + noise;  
    end
